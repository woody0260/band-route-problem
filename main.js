...
// Create an adjacency matrix to get all distance values between all points
let createAdjacencyMatrix = function () {
    adjacencyMatrix = Array(length).fill(0);
    ...
    for (var a = 0; a < length; a++) {
        for (var b = a; b < length; b++) {
            adjacencyMatrix[a][b] = Math.sqrt((pointsUsed[a].x - pointsUsed[b].x) * (pointsUsed[a].x - pointsUsed[b].x) + (pointsUsed[a].y - pointsUsed[b].y) * (pointsUsed[a].y - pointsUsed[b].y)); //pythagoras to get distance between points
            adjacencyMatrix[b][a] = adjacencyMatrix[a][b];
        }
    }
};

let simulatedAnnealing = function () {
    traversalPath = createFirstPath();
    let initialTemperature = length / 2;
    let distance = distanceCalc(traversalPath);
    draw(ctx2);
    for (let count = 0; count < steps; count++) {
        let temperature = (count/steps) * -1 * initialTemperature;
        let i = parseInt(length - 1 * Math.random());
        while (i === 0) {
            i = parseInt(length - 1 * Math.random());
        }

        let j = parseInt(length * Math.random());
        while (j === 0) {
           j = parseInt(length * Math.random()); 
        }

        let node1 = Math.min(i, j);
        let node2 = Math.max(i, j);

        let postNode2 = node2 === length-1 ? 0 : node2 + 1;
        
        var newDistance = adjacencyMatrix[traversalPath[node1]][traversalPath[node2]] + adjacencyMatrix[traversalPath[postNode2]][traversalPath[node1 + 1]] - adjacencyMatrix[traversalPath[node1]][traversalPath[node1 + 1]] - adjacencyMatrix[traversalPath[node2]][traversalPath[postNode2]];

        if (newDistance < 0 || Math.random() <= Math.exp((newDistance * -1) / temperature)) {
            let newPath = swapFunction(traversalPath, node1, node2);
            let newDistance = distanceCalc(newPath);
...
let distanceCalc = function (path) {
    let sum = 0;
    let returnHomeDistance = adjacencyMatrix[path[0]][path[length - 1]];
    for (let a = 1; a < length; a++) {
        sum = sum + adjacencyMatrix[path[a - 1]][path[a]];
    }
    return sum;
};


let swapFunction = function(array, posA, posB) {
    let temp = array[posA];
...
let drawPoints = function() {
    for (let i = 0; i < length; i++){
        ctx1.beginPath();
        ctx1.arc(pointsUsed[i].x, pointsUsed[i].y, 5, 0, 2 * Math.PI, true);
        if (i === 0) {
            ctx1.fillStyle = "#ff2626"
        } else {
